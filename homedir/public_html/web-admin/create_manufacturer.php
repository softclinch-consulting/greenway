...
$qry_validate = mysqli_query($con,"select * from `manufacturer` where `name`='$name' ") or die(mysqli_error($con));
...
$insert = mysqli_query($con,"insert into `manufacturer`(`name`,`desc`,`short_desc`,`post_name`,`post_date`,`order`,`status`,`c_date`)values('$name','$desc','$short_desc','$post_name','$post_date','$order','$status','$c_date')") or die(mysqli_error($con));
...
mkdir("../manufacturer/", 0755);
	   
$destination1 ='../manufacturer/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `manufacturer` set `image`='$imagename1' where `id`='$last'");

}
}

$msg ='<center><span style="color:#ff0000;">insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `manufacturer` set `name`='$name',`desc`='$desc',`short_desc`='$short_desc',`post_name`='$post_name',`post_date`='$post_date',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `manufacturer` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='manufacturer_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `manufacturer` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Manufacturer <span style="float: right;"><a href="manufacturer_list.php">Manufacturer List</a></span></h2>
		<br />
