...
$qry_validate = mysqli_query($con,"select * from `duration` where `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">duration already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `duration`(`name`,`order`,`status`,`c_date`)values('$name','$order','$status','$c_date')") or die(mysqli_error($con));

$msg ='<center><span style="color:#ff0000;">duration insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `duration` set `c_id`='$c_id',`s_id`='$s_id',`name`='$name',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));

//$msg ='<center><span style="color:#ff0000;">Admin Updated Successfully<span></center>';
?>
<script type="text/javascript">
//alert('Delete Successfully');
window.location='duration_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `duration` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
		...
		<hr />
					...
					<ol class="breadcrumb bc-3" >
						...
						<li class="active">
									...
									<strong>Create Duration</strong>
							</li>
							</ol>
					
		<h2>Create Duration <span style="float: right;"><a href="duration_list.php">Duration List</a></span></h2>
		<br />
...
<?php } ?>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="Duration Name" required="required" value="<?php echo $select1['name']; ?>">
								</div>
							</div>
