...
if (isset($_REQUEST['create_admin'])) {
	...
	if ($_REQUEST['edit_id'] == '') {
		...
		$insert = mysqli_query($con, "insert into `client`(`name`,`order`,`status`,`c_date`)values('$name','$order','$status','$c_date')") or die(mysqli_error($con));
		...
		if ($_FILES['image']['tmp_name']) {
			...
			if ($file_type != "image/jpeg" || $file_type != "image/png") {
				...
				mkdir("../client/", 0755);
				$destination1 = '../client/' . $imagename1;
				move_uploaded_file($source_image1, $destination1);
				mysqli_query($con, "update `client` set `image`='$imagename1' where `id`='$last'");

			}
		}
		$msg = '<center><span style="color:#ff0000;">Client Insert Successfully<span></center>';
	} else {
		$update = mysqli_query($con, "update `client` set `name`='$name',`order`='$order',`status`='$status' where `id`='" . $_REQUEST['edit_id'] . "' ") or die(mysqli_error($con));
		...
		if ($_FILES['image']['tmp_name']) {
			...
			if ($file_type != "image/jpeg" || $file_type != "image/png") {
				...
				$destination1 = '../client/' . $imagename1;
				move_uploaded_file($source_image1, $destination1);
				mysqli_query($con, "update `client` set `image`='$imagename1' where `id`='" . $_REQUEST['edit_id'] . "'");
			}
		}
		...
		<script type="text/javascript">
			...
			window.location = 'client_list.php?msg=updated';
		</script>
		<?php
	}
...
if (isset($_REQUEST['e_id'])) {
	$select = mysqli_query($con, "select * from `client` where `id`='" . $_REQUEST['e_id'] . "' ") or die(mysqli_error($con));
...
<body class="page-body" data-url="http://neon.dev">
	...
	<div class="page-container">
		...
		<div class="main-content">
			...
			<h2>Add Client <span style="float: right;"><a href="client_list.php">Client List</a></span></h2>
			<br />
						...
						<div class="panel-body">
							...
							<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST"
								...
								<div class="form-group">
									...
									<div class="col-sm-5">
										<input type="file" class="form-control" id="field-1" name="image"
											required="required" value="<?php echo $select1['image']; ?>">
									</div>
								</div>
								...
								<div class="form-group" style="display: none;">
									...
									<div class="col-sm-5">
										<input type="text" class="form-control" id="field-1" name="link"
											placeholder="Video Link" value="<?php echo $select1['link']; ?>">
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-5">
										<input type="number" class="form-control" id="field-1" name="order"
											placeholder="Client order" required="required"
