...
$qry_validate = mysqli_query($con,"select * from `blog` where `name`='' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">Blog already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `blog`(`name`,`desc`,`short_desc`,`post_name`,`post_date`,`order`,`status`,`c_date`)values('$name','$desc','$short_desc','$post_name','$post_date','$order','$status','$c_date')") or die(mysqli_error($con));
...
$destination1 ='../blog/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `blog` set `image`='$imagename1' where `id`='$last'");
...
mkdir("../blog_icon/", 0755);
	   
$destination2 ='../blog_icon/'.$imagename2;
	        
move_uploaded_file($source_image2,$destination2);

mysqli_query($con,"update `blog` set `image_icon`='$imagename2' where `id`='$last'");
...
$msg ='<center><span style="color:#ff0000;">Blog insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `blog` set `type`='$type',`c_id`='$c_id',`name`='$name',`desc`='$desc',`short_desc`='$short_desc',`post_name`='$post_name',`post_date`='$post_date',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `blog` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
}
}
...
mysqli_query($con,"update `blog` set `image_icon`='$imagename2' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='blog_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `blog` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Blog <span style="float: right;"><a href="blog_list.php">Blog List</a></span></h2>
		<br />
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
                            ...
                            <!--<div class="form-group">
								<label class="col-sm-3 control-label">Type <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<select class="form-control" name="type" required="required">
										
						<option value="News" <?php if($select1['type']=='News'){?> selected="selected" <?php } ?> >News</option>
						<option value="Events" <?php if($select1['type']=='Events'){?> selected="selected" <?php } ?> >Events</option>
						<option value="Support" <?php if($select1['type']=='Support'){?> selected="selected" <?php } ?> >Support</option>
									</select>
								</div>
							</div>--->
							...
							<!--<div class="form-group">
								...
								<div class="col-sm-5">
								<select class="form-control" name="c_id"  onchange="category(this.value,'category');">
										
						<option value="">Select</option>
                        <?php
                        $category  = mysqli_query($con,"select * from `blog_cat` order by `name` asc ")or die(mysqli_error($con));
					    ...
					    ?>
							...
							<div class="form-group" style="display: none;">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="short_desc" placeholder="Short Description"  value="<?php echo $select1['short_desc']; ?>">
								</div>
							</div>
							...
							<div class="form-group" style="display: none;">
								...
								<div class="col-sm-9">				
	                             <textarea name="desc2" class="form-control ckeditor" style="height: 150px;" placeholder="Description Two" ><?php echo $select1['desc2']; ?></textarea>
								</div>
							</div>


							<!--<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Image Icon <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="file" class="form-control" id="field-1" name="image_icon" value="<?php echo $select1['image_icon']; ?>">
								</div>
							</div>-->
