...
$qry_validate = mysqli_query($con,"select * from `admin` where `user`='$user' ") or die(mysqli_error($con));
...
$insert = mysqli_query($con,"insert into `admin`(`name`,`email`,`mobile`,`type`,`location`,`user`,`pass`,`status`,`c_date`)values('$name','$email','$mobile','$type','$location','$user','$pass','$status','$c_date')") or die(mysqli_error($con));

$msg ='<center><span style="color:#ff0000;">Admin Create Successfully<span></center>';
}
...
$update = mysqli_query($con,"update `admin` set `name`='$name',`email`='$email',`mobile`='$mobile',`type`='$type',`location`='$location',`user`='$user',`pass`='$pass',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));

//$msg ='<center><span style="color:#ff0000;">Admin Updated Successfully<span></center>';
?>
<script type="text/javascript">
//alert('Delete Successfully');
window.location='admin_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `admin` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Admin <span style="float: right;"><a href="admin_list.php">Admin List</a></span></h2>
		<br />
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<hr>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="password" class="form-control" name="pass" id="field-3" placeholder="Password" required="required" value="<?php echo $select1['pass']; ?>">
								</div>
							</div>
