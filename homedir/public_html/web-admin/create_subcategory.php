...
$qry_validate = mysqli_query($con,"select * from `sub_category` where `c_id`='$c_id' and  `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">sub category already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `sub_category`(`c_id`,`name`,`order`,`status`,`c_date`)values('$c_id','$name','$order','$status','$c_date')") or die(mysqli_error($con));
...
mkdir("../sub_category/", 0755);
	   
$destination1 ='../sub_category/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `sub_category` set `image`='$imagename1' where `id`='$last'");

}
}

$msg ='<center><span style="color:#ff0000;">sub category Insert Successfully<span></center>';
}
...
$update = mysqli_query($con,"update `sub_category` set `c_id`='$c_id',`name`='$name',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `sub_category` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='subcategory_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `sub_category` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Sub Category <span style="float: right;"><a href="subcategory_list.php">Sub Category List</a></span></h2>
		<br />
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<div class="panel-body">
					    ...
					    ?>
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Sub Category Name <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="Sub Category Name" required="required" value="<?php echo $select1['name']; ?>">
								</div>
							</div>
							...
							<div class="form-group" style="display: none;">
								...
								<div class="col-sm-5">
									<input type="number" class="form-control" id="field-1" name="order" placeholder="Sub Category order"  value="<?php echo $select1['order']; ?>" min="0">
								</div>
							</div>
