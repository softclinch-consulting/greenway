...
$qry_validate = mysqli_query($con,"select * from `industries` where `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">Industries already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `industries`(`name`,`mrp`,`price`,`short_desc`,`desc`,`brand`,`review`,`order`,`status`,`c_date`)values('$name','$mrp','$price','$short_desc','$desc','$brand','$review','$order','$status','$c_date')") or die(mysqli_error($con));
...
$destination1 ='../industries/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `industries` set `image`='$imagename1' where `id`='$last'");

}
}


$msg ='<center><span style="color:#ff0000;">Industries insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `industries` set `name`='$name',`mrp`='$mrp',`price`='$price',`short_desc`='$short_desc',`desc`='$desc',`review`='$review',`brand`='$brand',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `industries` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='industries_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `industries` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
		...
		<hr />
					...
					<ol class="breadcrumb bc-3" >
						...
						<li class="active">
									...
									<strong>Create Industries</strong>
							</li>
							</ol>
					
		<h2>Create Industries <span style="float: right;"><a href="industries_list.php">Industries List</a></span></h2>
		<br />
...
xmlhttp.send();
	...
	<textarea name="desc" class="form-control ckeditor" placeholder="Description" ><?php echo $select1['desc']; ?></textarea>
							...
							<!--<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Industries Image <span style="color:#ee4749">*</span></label>
