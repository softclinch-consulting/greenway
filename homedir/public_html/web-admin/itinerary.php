...
{
$select  = mysqli_query($con,"delete from `listing_itinerary` where `id`='".$_REQUEST['d_id']."' ")or die(mysqli_error($con));
?>
<script type="text/javascript">
alert('Delete Successfully');
window.location='itinerary.php?l_id=<?php echo $_REQUEST['l_id']; ?>';
</script>
<?php
}
...
/*$qry_validate = mysqli_query($con,"select * from `listing_itinerary` where `name`='$name' ") or die(mysqli_error($con));
...
$insert = mysqli_query($con,"insert into `listing_itinerary`(`l_id`,`title`,`day`,`desc`,`order`,`status`,`date`)values('$l_id','$title','$day','$desc','$order','$status','$date')") or die(mysqli_error($con));


$msg ='<center><span style="color:#ff0000;">itinerary insert successfully<span></center>';
//}
...
$update = mysqli_query($con,"update `listing_itinerary` set `l_id`='$l_id',`title`='$title',`day`='$day',`desc`='$desc',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));


//$msg ='<center><span style="color:#ff0000;">itinerary Updated Successfully<span></center>';
?>
<script type="text/javascript">
...
{
$select  = mysqli_query($con,"select * from `listing_itinerary` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		<hr />
					...
					<ol class="breadcrumb bc-3" >
						...
						<li class="active">
									...
									<strong>Create Itinerary</strong>
							</li>
							</ol>
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
						...
						<input type="hidden" name="l_id" value="<?php echo $_REQUEST['l_id']; ?>">
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Day<span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="day" placeholder="Day" required="required" value="<?php echo $select1['day']; ?>">
								</div>
							</div>
		...
		<table class="table table-bordered datatable" id="table-4">
			...
			<tbody>
				...
				<?php
				$select  = mysqli_query($con,"select * from `listing_itinerary` where `id`!='' and `l_id`='".$_REQUEST['l_id']."' order by `id` desc ")or die(mysqli_error($con));
				$i='1';
				...
				<tr class="odd gradeX">
					...
					<td class="sorting_1">
						<a href="itinerary.php?e_id=<?php echo $select1['id']; ?>&l_id=<?php echo $_REQUEST['l_id']; ?>" class="btn btn-default btn-sm btn-icon icon-left">
							<i class="entypo-pencil"></i>
							Edit
						</a>
						<?php //if($select1[type]=='1'){ ?>
						<a href="javascript:confirmDelete('itinerary.php?d_id=<?php echo $select1['id']; ?>&l_id=<?php echo $_REQUEST['l_id']; ?>')" class="btn btn-danger btn-sm btn-icon icon-left">
