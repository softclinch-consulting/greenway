...
$qry_validate = mysqli_query($con,"select * from `room_type` where `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">room type already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `room_type`(`name`,`order`,`status`,`c_date`)values('$name','$order','$status','$c_date')") or die(mysqli_error($con));
...
$update = mysqli_query($con,"update `room_type` set `name`='$name',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));

//$msg ='<center><span style="color:#ff0000;">Admin Updated Successfully<span></center>';
?>
<script type="text/javascript">
//alert('Delete Successfully');
window.location='room_type_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `room_type` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Room Type <span style="float: right;"><a href="room_type_list.php">Room Type List</a></span></h2>
		<br />
					...
					<div class="panel-body">
						...
						<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST">
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Room Type Name <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="Room Type Name" required="required" value="<?php echo $select1['name']; ?>">
								</div>
							</div>
							...
							<div class="form-group" style="display: none;">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="order" placeholder="Room Type order" required="required" value="<?php echo $select1['order']; ?>">
								</div>
							</div>
