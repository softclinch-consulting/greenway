...
$qry_validate = mysqli_query($con,"select * from `certificate` where `name`='$user' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">Certificate already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `certificate`(`name`,`order`,`status`,`c_date`)values('$name','$order','$status','$c_date')") or die(mysqli_error($con));
...
$destination1 ='../certificate/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `certificate` set `image`='$imagename1' where `id`='$last'");

}
}


$msg ='<center><span style="color:#ff0000;">Certificate Insert Successfully<span></center>';
}
...
$update = mysqli_query($con,"update `certificate` set `name`='$name',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `certificate` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='certificate_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `certificate` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Add Certificate <span style="float: right;"><a href="certificate_list.php">Certificate List</a></span></h2>
		<br />
			...
			<div class="col-md-12">
				...
				<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="Certificate Name" required="required" value="<?php echo $select1['name']; ?>">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="number" class="form-control" id="field-1" name="order" placeholder="Certificate order" required="required" value="<?php echo $select1['order']; ?>" min="0">
								</div>
							</div>
