...
$qry_validate = mysqli_query($con,"select * from `add_image` where `name`='$user' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">Add image already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `add_image`(`p_id`,`name`,`short_description`,`order`,`status`,`c_date`)values('$p_id','$name','$short_description','$order','$status','$c_date')") or die(mysqli_error($con));
...
mkdir("../add_image/", 0755);
	   
$destination1 ='../add_image/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `add_image` set `image`='$imagename1' where `id`='$last'");
...
$msg ='<center><span style="color:#ff0000;">Add image Insert Successfully<span></center>';
}
...
$update = mysqli_query($con,"update `add_image` set `p_id`='$p_id',`name`='$name',`short_description`='$short_description',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `add_image` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
}
}
...
window.location='add_image_list.php?msg=updated&id=<?php echo $_REQUEST['id']; ?>';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `add_image` where `id`='$_REQUEST[e_id]' ")or die(mysqli_error($con));
		...
		<hr />
					...
					<ol class="breadcrumb bc-3" >
						...
						<li class="active">
									...
									<strong>Create Add Image</strong>
							</li>
							</ol>
					
		<h2>Create Add Image <span style="float: right;"><a href="add_image_list.php?id=<?php echo $_REQUEST['id']; ?>">Add Image List</a></span></h2>
		<br />
			...
			<div class="col-md-12">
				...
				<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
			                 ...
			                 <div class="form-group">
									...
									<input type="hidden" class="form-control" id="field-1" name="p_id" placeholder="Add Image Name" required="required" value="<?php echo $_REQUEST['id']; ?>">
								
							</div>

							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Add Image Name <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="Add Image Name" required="required" value="<?php echo $select1['name']; ?>">
								</div>
							</div>
