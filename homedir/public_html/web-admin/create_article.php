...
$insert = mysqli_query($con,"insert into `article`(`type`,`name`,`meta_title`,`meta_key`,`meta_desc`,`short_desc`,`desc`,`desc1`,`post_name`,`post_date`,`order`,`status`,`c_date`)values('$type','$name','$meta_title','$meta_key','$meta_desc','$short_desc','$desc','$desc1','$post_name','$post_date','$order','$status','$c_date')") or die(mysqli_error($con));
...
$destination1 ='../article/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `article` set `image`='$imagename1' where `id`='$last'");
...
$update = mysqli_query($con,"update `article` set `type`='$type',`name`='$name',`meta_title`='$meta_title',`meta_key`='$meta_key',`meta_desc`='$meta_desc',`short_desc`='$short_desc',`desc`='$desc',`desc1`='$desc1',`post_name`='$post_name',`post_date`='$post_date',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `article` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='article_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `article` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
			...
			<div class="col-md-12">
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
                        ...
                        <?php
                        $article  = mysqli_query($con,"select * from `article` order by `name` asc ")or die(mysqli_error($con));
                        while ($article1 = mysqli_fetch_array($article)) 
                        {
                        ?>
						<option value="<?php echo $article1['id']; ?>" <?php if($select1['type']==$article1['id']){?> selected="selected" <?php } ?> ><?php echo $article1['name']; ?></option>
						<?php
					    }
					    ?>
							...
							<div class="form-group" style="display:none;">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="meta_title" placeholder="Meta Title" value="<?php echo $select1['meta_title']; ?>">
								</div>
							</div>
							...
							<div class="form-group" style="display:none;">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="meta_key" placeholder="Meta Keyword"  value="<?php echo $select1['meta_key']; ?>">
								</div>
							</div>
							...
							<div class="form-group" style="display:none;">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="meta_desc" placeholder="Meta Description"  value="<?php echo $select1['meta_desc']; ?>">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">				
	                            <textarea name="desc1" class="form-control ckeditor" placeholder="Description 2" required="required"><?php echo $select1['desc1']; ?></textarea>
								</div>
							</div>
							...
							<div class="form-group" style="display:none;">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="post_name" placeholder="Post Name"  value="<?php echo $select1['post_name']; ?>">
								</div>
							</div>
