...
$qry_validate = mysqli_query($con,"select * from `product` where `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">product already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `product`(`name`,`mrp`,`price`,`short_desc`,`desc`,`desc1`,`brand`,`review`,`seo_url`,`order`,`status`,`c_date`)values('$name','$mrp','$price','$short_desc','$desc','$desc1','$brand','$review','$seo_url','$order','$status','$c_date')") or die(mysqli_error($con));
...
$destination1 ='../product/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `product` set `image`='$imagename1' where `id`='$last'");

}
}


$msg ='<center><span style="color:#ff0000;">product insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `product` set `name`='$name',`mrp`='$mrp',`price`='$price',`short_desc`='$short_desc',`desc`='$desc',`desc1`='$desc1',`review`='$review',`brand`='$brand',`seo_url`='$seo_url',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `product` set `image`='$imagename1' where `id`='".$_REQUEST['edit_id']."'");
...
window.location='product_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `product` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Product <span style="float: right;"><a href="product_list.php">Product List</a></span></h2>
		<br />
...
xmlhttp.send();
	...
	<textarea name="desc" class="form-control ckeditor" placeholder="Description" ><?php echo $select1['desc']; ?></textarea>
								...
								<div class="form-group" >
								<label for="field-1" class="col-sm-3 control-label">Description 1<span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-9">				
	<textarea name="desc1" class="form-control ckeditor" placeholder="Description" ><?php echo $select1['desc1']; ?></textarea>
								</div>
							</div>-->
							...
							<!--<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Product Image <span style="color:#ee4749">*</span></label>
							...
							<!--<div class="form-group" style="display: none;">
								<label for="field-1" class="col-sm-3 control-label">Reviews<span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="review" placeholder="Review"  value="<?php echo $select1['review']; ?>">
								</div>
							</div>

							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label"> MRP Price<span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="mrp" placeholder="price" required="required" value="<?php echo $select1['mrp']; ?>">
								</div>
							</div>


							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Price<span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="price" placeholder="price" required="required" value="<?php echo $select1['price']; ?>">
								</div>
							</div>-->
							...
							<!--<div class="form-group" >
								<label for="field-1" class="col-sm-3 control-label">SEO URL</label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="seo_url" placeholder="SEO Canonical Link" value="<?php echo $select1['seo_url']; ?>">
								</div>
							</div>-->
