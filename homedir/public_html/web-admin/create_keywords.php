...
$insert = mysqli_query($con,"insert into `keywords`(`s_id`,`c_id`,`dt_id`,`l_id`,`meta_title`,`meta_key`,`meta_desc`,`order`,`status`,`c_date`)values('$s_id','$c_id','$dt_id','$l_id','$meta_title','$meta_key','$meta_desc','$order','$status','$c_date')") or die(mysqli_error($con));

$msg ='<center><span style="color:#ff0000;">Keywords Insert Successfully<span></center>';
...
$update = mysqli_query($con,"update `keywords` set `s_id`='$s_id',`dt_id`='$dt_id',`c_id`='$c_id',`l_id`='$l_id',`meta_title`='$meta_title',`meta_key`='$meta_key',`meta_desc`='$meta_desc',`order`='$order',`status`='$status' where `id`='".$_REQUEST['edit_id']."' ")or die(mysqli_error($con));

//$msg ='<center><span style="color:#ff0000;">Admin Updated Successfully<span></center>';
?>
<script type="text/javascript">
//alert('Delete Successfully');
window.location='keywords_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `keywords` where `id`='".$_REQUEST['e_id']."' ")or die(mysqli_error($con));
		...
		<hr />
					...
					<ol class="breadcrumb bc-3" >
						...
						<li class="active">
									...
									<strong>Create Keywords</strong>
							</li>
							</ol>
					
		<h2>Create Keywords <span style="float: right;"><a href="keywords_list.php">Keywords List</a></span></h2>
		<br />
...
xmlhttp.open("GET", "ajax_page.php?dt_id=" + dt_id + "&type=" + type, false);
xmlhttp.send();
}
</script>
					    ...
					    ?>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
								   ...
								   <?php } else{ ?>
									<select class="form-control" name="dt_id" id="district_result" required="required" onchange="district(this.value,'district');">
...
<?php } ?>
								...
								<div class="col-sm-5">
									<?php if($_REQUEST['e_id']==''){ ?>
									<span id="location_result">Choose District</span>
								   <?php } else{ ?>
									<select class="form-control" name="l_id" id="location_result" required="required">
										
<option value="">Select</option>
<?php
$location = mysqli_query($con,"select * from `location` where `dt_id`='$select1[dt_id]' order by `name` asc ")or die(mysqli_error($con));
...
<?php } ?>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="meta_title" placeholder="Title" required="required" value="<?php echo $select1['meta_title']; ?>">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									...
									<textarea name="meta_key" class="form-control" rows="4" placeholder="Keywords" ><?php echo $select1['meta_key']; ?></textarea>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									...
									<textarea name="meta_desc" class="form-control" rows="5" placeholder="Description" ><?php echo $select1['meta_desc']; ?></textarea>
							...
							<!--<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Key Words <span style="color:#ee4749">*</span></label>
								
							<div class="col-sm-5">
									<textarea name="keywords" class="form-control" rows="4" placeholder="Key Words" ><?php echo $select1['keywords']; ?></textarea>
								</div>
							</div>-->
