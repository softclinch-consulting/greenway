...
$insert = mysqli_query($con,"insert into `achiever`(`c_id`,`name`,`position`,`location`,`order`,`status`,`c_date`)values('$c_id','$name','$position','$location','$order','$status','$c_date')") or die(mysqli_error($con));
...
mkdir("../Achievers/", 0755);
	   
$destination1 ='../Achievers/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `achiever` set `image`='$imagename1' where `id`='$last'");

}	
		


$msg ='<center><span style="color:#ff0000;">Achievers Insert Successfully<span></center>';
}
...
$update = mysqli_query($con,"update `achiever` set  `c_id`='$c_id',`name`='$name',`position`='$position',`location`='$location',`order`='$order',`status`='$status' where `id`='$_REQUEST[edit_id]' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `achiever` set `image`='$imagename1' where `id`='$_REQUEST[edit_id]'");
...
window.location='achiever_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `achiever` where `id`='$_REQUEST[e_id]' ")or die(mysqli_error($con));
		...
		<hr />
					...
					<ol class="breadcrumb bc-3" >
						...
						<li class="active">
									...
									<strong>Create Achievers</strong>
							</li>
							</ol>
					
		<h2>Create Achievers <span style="float: right;"><a href="achiever_list.php">Achievers List</a></span></h2>
		<br />
			...
			<div class="col-md-12">
				...
				<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group">
								<label class="col-sm-3 control-label">Exam Category <span style="color:#ee4749">*</span></label>
                        ...
                        <?php
                        $category  = mysqli_query($con,"select * from `exam_category` order by `name` asc ")or die(mysqli_error($con));
